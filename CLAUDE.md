...
**Read before working**: `docs/ROADMAP.md` (epics, milestones, engineering standards), `docs/DECISIONS.md` (ADR-001…028 — ADR-012 pins the byte-level DST semantics and lists known reference bugs never to port; ADR-013 pins color-change flag placement; ADR-015 pins thread-color emission semantics; ADR-016 pins the M2 ProgramModel/Interpreter target split; ADR-017 pins formula-arithmetic divergences; ADR-018 pins interpreter tick/clock semantics; ADR-019 pins how a golden sitting on a threshold must guard itself; ADR-020 pins the engine's coordinate boundary; ADR-021 pins the M3 preview data path; ADR-022 pins the M3 target layout; ADR-023 pins what runs where — the local commit gate compiles the app, CI runs it; ADR-024 pins the stage renderer; ADR-025 pins the DST header's field widths as a serialization error rather than a trap, and pins field emission order as a contract; ADR-026 pins DST export — the `exportModel.count > 1` gate, the rejecting `DesignName` in front of the mangling header backstop, eager preparation (so a US-211 overflow is a gate reason rather than a failed share), the per-session temp directory, and the exported `.dst` UTType; ADR-027 pins the run lifecycle — two tasks with producer-only cancellation, a terminal that cannot omit the export model, and three budget axes; ADR-028 pins zoom/pan and the stage's VoiceOver summary — one commit per gesture, a fit-aware zoom floor, and a summary only a run-state transition may write), and the current milestone's stories in `docs/user-stories/`.
...
- **Knowledge-graph drift check at milestone close** — run `/graphify . --update` alongside the `swift-documenter` pass, and **not** per story. It answers one question this toolchain otherwise never asks: *has a new story reinvented an invariant an existing ADR already owns?* Read the `semantically_similar_to` edges that cross the `docs/` communities and triage them one by one — **the step is done when those edges have been triaged, not when the tool has been run** — then journal the outcome, including "none, N edges triaged". The AST layer is deterministic and needs no LLM or API key; the semantic layer over `docs/` is what costs (~850k tokens on the first full build; `--update` re-extracts only changed files), which is why this is a milestone-boundary tool. **The graph never wins against an ADR** — its `INFERRED`/`AMBIGUOUS` edges are a subagent's single-pass reading of the docs, not a source of truth; where a graph claim and an ADR disagree, the ADR is right and the graph is stale or wrong. Never cite a graph edge to justify changing a golden test (see process rule 3). `graphify-out/` is derived: only `GRAPH_REPORT.md` is tracked (it is the part that diffs meaningfully between milestones); `graph.json`, the HTML viewer and the extraction cache are ignored.
...
- **Cross-vendor review is automated**: creating a PR triggers a hook reminder to run `/codex-review` (OpenAI Codex, read-only, adversarial-semantics rubric — deliberately different from `swift-code-reviewer`'s). Triage its findings against ADR-012/013 (the ADRs win over the reviewer), record the verdict in the PR description, and journal the delegation outcome. If the triage changed the branch, run the skill's verification round (Codex re-reviews its own fixes). **The loop ends when a round produces no code changes, or when severity has fallen for two consecutive rounds — not on a fixed count**; a flat severity run is not convergence. Hard cap 10, and escalate early if severity stays flat or rises for three rounds. Record each round's highest severity, since the stop condition depends on that history. `/finish` verifies this happened; a PR is handed over only with the final round's verdict recorded.
...
- **Never commit red** — except the deliberate TDD red phase: a PreToolUse hook in `.claude/settings.json` runs the engine tests before any `git commit`, plus a signing-free app compile whenever the working tree holds any change under `catrobat_embroidery_ios/`, and blocks if either fails. A story's failing-tests-first commit is marked with a literal `[red]`, which skips the gate so CI shows the red baseline (the hook tests for `[red]` anywhere in the command — it is an escape hatch, not a lock). **The local hook is a convenience that catches mistakes, not an enforcement boundary** (ADR-023): it matches command *text*, so unusual command forms and shell expansion can slip past it, and `[red]` is available to anyone who wants it. What actually keeps red out of `main` is the required-status-checks ruleset — engine tests, SwiftLint and the app build must pass before a PR can merge.
