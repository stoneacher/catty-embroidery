...
- **Never commit red** — except the deliberate TDD red phase: a PreToolUse hook in `.claude/settings.json` runs the engine tests before any `git commit`, plus a signing-free app compile when the commit touches `catrobat_embroidery_ios/`, and blocks if either fails. A story's failing-tests-first commit is marked with a literal `[red]` in the commit message, which skips the gate so CI shows the red baseline. **The local hook is a convenience that catches mistakes, not an enforcement boundary** (ADR-023): it matches command *text*, so unusual command forms and shell expansion can slip past it, and `[red]` is available to anyone who wants it. What actually keeps red out of `main` is the required-status-checks ruleset — engine tests, SwiftLint and the app build must pass before a PR can merge.
