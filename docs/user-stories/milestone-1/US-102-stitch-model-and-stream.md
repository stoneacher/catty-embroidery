...
## Acceptance criteria
- [x] `Stitch`: position (embroidery coordinates), thread color, flags `isJump` and `isColorChange` — a `Sendable`, `Equatable` value type (strict concurrency is on; domain types must cross isolation boundaries freely).
- [x] Coordinate conversion: stage points → embroidery units with factor 2.0 (`STITCH_POINT_UNIT_FACTOR`), rounding = `floor(x + 0.5)` matching Java `Math.round` (differs from Swift `.rounded()` on negative halves — ADR-012). **No y-flip**: stage y-up maps straight to DST +Y in both references; flipping for screen display is M3's rendering concern. Stage space per ADR-007 (center origin, 500×500 pt).
- [x] `EmbroideryStream`: append stitches, append jump, append color change; exposes stitch count, color-change count (header `CO` = count + 1, see US-104), bounding box (min/max X/Y), and **first and last stitch positions** (the header's AX/AY need them).
- [x] `EmbroideryStream` is a plain `Sendable` struct with `mutating` appends — do not port Catty's class + `SynchronizedArray` + draw-queue design; M3's preview only needs "stitches since index i", which an array gives for free.
- [x] No UIKit/SwiftUI/SpriteKit imports — pure Foundation.
