...
## Acceptance criteria
- [x] Encoder converts a delta (dx, dy in embroidery units, each within ±121 **inclusive** — Catty's strict-comparison guard that rejects legal ±121 is a bug, do not port; ADR-012) into the 3-byte DST record. Port Catroid's 243-entry `CONVERSION_TABLE` **verbatim as data** (AGPL-compatible, provenance comment) rather than hand-deriving the encoder from the bit diagram; the algorithmic derivation exists only as a test that regenerates the table.
- [x] Relative deltas are computed **between individually converted absolute positions** (round-then-subtract), never by converting the difference (ADR-012).
- [x] Byte 3 flags: jump stitch sets `0x80`; color change sets `0xC0`; plain stitch sets the base bits per spec.
- [x] Deltas outside ±121 are rejected at this layer via `precondition` (unreachable once US-105's interpolation is a stream invariant), tested.
- [x] Decoder for the same 3-byte records lives in the **test target only** — no product feature reads DST in M1–M6 (if E7's export validation ever means re-reading files, promote it then as its own story). Encode/decode round-trips.
