...
- [x] Setting a color that **differs** from the current thread color emits a DST color-change record (deliberate divergence from Android, where the brick never emits machine-level changes — ADR-012); setting the same color is a no-op.
