...
- [ ] A move from P to Q whose delta exceeds 121 embroidery units on either axis emits, in order: a **duplicate of P flagged as jump**, the evenly spaced intermediates flagged as jumps, **Q flagged as jump**, then **Q again as a plain stitch** (delta 0).
- [ ] `splitCount = ceil(maxAxisDistanceInUnits / 121)`; intermediate positions are computed and **rounded in stage coordinates before** the ×2 unit conversion (ADR-012 rounding rule).
- [ ] Moves with both axes ≤121 units pass through unmodified (no duplicates).
- [ ] Transitions between disjoint path segments (needle repositioning without stitching) are emitted as jump stitches.

## Test-first plan
1. Boundary: delta 121 → single plain stitch, no interpolation. Delta 122 → dup-of-previous (jump), target (jump), target (plain) — 3 records for the move.
