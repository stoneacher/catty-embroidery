...
- [ ] A CI job `app-build-and-test` on `macos-26` with the same pinned `DEVELOPER_DIR` as the engine job, running `xcodebuild test -scheme catrobat_embroidery_ios -destination 'platform=iOS Simulator,name=iPhone 17'`, added to the required checks. Depends on the shared scheme existing — `xcshareddata/xcschemes/` is absent today and a fresh CI checkout cannot be relied on to autocreate it.
- [ ] The local PreToolUse commit gate **keeps `swift test` engine-only** and gains a signing-free app *compile* check — `xcodebuild build -destination 'generic/platform=iOS' CODE_SIGNING_ALLOWED=NO` with a persistent `-derivedDataPath` outside the repo — conditioned on the staged diff touching `catrobat_embroidery_ios/`. Measured at 9.3 s cold on the pre-link target; **re-measure with five products linked and drop the check if it exceeds ~30 s incremental.** A simulator boot in a pre-commit hook is explicitly rejected: it would cost the small-commit cadence the whole process rests on.
