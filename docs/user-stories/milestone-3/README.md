...
**Ruling at milestone close (2026-09-19): two of three criteria met, the third met on one path and recorded unmet on the other.** End-to-end sample → preview → DST works and is verified in the running app rather than only in tests (US-308 — `LA:SquareCoil`, `ST:2976`, `CO:2` in the exported bytes). The zoom/pan transform math is unit-tested in a Foundation-only target with no simulator (US-302, extended by US-313a's `StageManipulation`). The **60 fps criterion is met on the animating path and unmet on the mid-gesture path**: animating to 50 000 reads `p99 16.670 ms` over drawn frames, while the cleanest mid-gesture capture yet (2026-09-18, `drawn=987/1018`, `commits=1`) still has a worst frame of **38.076 ms** against a 33.3 ms bar. **What the milestone knows now and did not at US-309 is that neither rung of ADR-029's ladder is the lever.** Rung 1 cannot touch a path that never bakes; rung 2 landed in US-310, moved the median from 69.1 ms to one refresh period on device, and did not move the tail at any of four stride values; and US-313b's AC12/AC13 controls then closed the door from the other side — a 3 194-stitch design drawing *more* uncoarsened geometry than the coarsened 50 001-stitch one sits at the floor, and halving the coarsened count changes nothing. **The mid-gesture frame cost is not the number of segments drawn.** The leading hypothesis is fill rate and overdraw, and the fixture that would discriminate it does not exist in this repo. That is **US-316**, and it is the honest state of the criterion: not "nearly met", but measured, attributed twice to the wrong cause, and now pointed at a named experiment.
...
- [ ] **The A15-class capture.** US-309 measured on an iPhone 17 Pro (A19). That is enough to
      ...
      **Disposition at milestone close (2026-09-19): carried out of M3, unticked.** It leaves as
      the one verification item never run, and it is worth *less* now than when it was written:
      US-313b's controls showed the mid-gesture cost is not the segment count, so an A15 capture
      today would confirm a fix that has not been made. The order it wants is therefore inverted —
      **US-316 first, then A15**: measure the thing that is actually slow, then confirm it on the
      hardware the criterion names. An A19 pass still says nothing about an A15, which is why
      three sessions on that device have not ticked this box.
