...
- [ ] Selecting a sample resets the run and the display list; re-selecting the *same* sample resets rather than resumes. There is no cross-sample state.
...
2. Selecting a sample produces a `RunViewModel` (or its inputs) for that sample's `Program`.
3. Re-selecting the currently selected sample resets rather than resumes.
