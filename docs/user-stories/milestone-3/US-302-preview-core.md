...
9. Multi-actor clause-B case: two actors on one layer, and the assembled stream's **two** black points at the first actor's workspace position are absent from the display list. Asserts the divergence rather than the agreement, and asserts the count — "some black points appear" would pass against a one-point implementation too.
