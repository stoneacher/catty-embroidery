...
| T3 | At `SyntheticDesign.displayList(count: 50_000, colorRuns: 5)`, budget 4 000: thread segments `≤ ceil(50_000/13) + 5 + fineTraversalCount` and dots `≤ ceil(50_000/13) + 5`, **and both ≥ 3 000** — two-sided, because a `coarse` returning nothing sails through an upper bound. `fineTraversalCount` is computed from `entire(of:)` in the test. **Superseded**: the shipped test adds `+ unreachableIntervals + corners`, without which this exact fixture reads 4 245 against a bound of 3 852 — see AC8 and ADR-030. | Yes (compile). | k forced to 1 → far over the bound. `strokes(for:)` returning `[]` → the lower bound fails. |
