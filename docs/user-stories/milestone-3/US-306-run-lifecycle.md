...
5. **Oversize single tick**, two cases, because one thread does not exercise the real bound: (a) a single triple-stitch move engineered to emit far more than `maxStitchesPerFrame` in one `step()`; (b) **several concurrent scripts** each emitting a large batch in the same tick, since `step()` sums over all runnable threads and a one-thread test would pass against an implementation that assumed a per-thread ceiling. Both are applied whole, in one observable mutation, without dropping events or deadlocking.
