...
**Mutation targets**: **subtracting the block length unconditionally** rather than counting removed indices below `toOffset` — which is *correct* downward (`[R,A,E,B]`: 4 − 3 = 1) and **wrong upward** (`[B,R,A,E]`: 0 − 3 = −3), so **test 3 is the one that catches it** and test 2 passes. *(The first draft predicted this backwards — "green for upward, wrong for downward"; Codex round 4. Fourth false mutation prediction in this plan, see the journal.)* Also: the sibling jump using `matchingEnd` of the wrong index in a nested script.
