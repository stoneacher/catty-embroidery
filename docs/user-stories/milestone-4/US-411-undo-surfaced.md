...
**The plain toolbar undo/redo pair lands earlier, in US-408**, because that story introduces destructive deletion whose justification depends on undo being reachable. What remains here is the `UndoManager` bridge, the VoiceOver announcements and the totality proof. **The run/export/autosave consequences of a history transition also ship in US-408**, with the buttons that trigger them — the criteria below re-verify them through the bridge rather than introducing them. The first criterion below is therefore a **restatement of what US-408 shipped**, verified rather than built.

## The `UndoManager` constraint, verified rather than assumed

**`EnvironmentValues.undoManager` is get-only** — confirmed in the iOS 27 SDK's `SwiftUI.swiftinterface` (line 21668), where the property declares only a `get`. So `.environment(\.undoManager, myManager)` **does not compile**, and the manager the environment surfaces comes from the responder chain. It may also be `nil`. Note it does **not** follow that the manager is not ours: `UIHostingController.undoManager` is an overridden getter, and an overridden responder getter can return one we own. The design does not depend on either reading.
...
- [ ] **The bridge is synchronised after *every* history transition, not only on load.** `UndoManager` keeps its own operation history and does not observe the package stack, so a toolbar undo — which by the criterion above must drive the package stack directly — leaves the manager holding a registration the stack no longer has. Concrete case: make edits A and B, undo B from the toolbar; the manager still holds B's undo registration with no matching redo, so system Redo is unavailable and system Undo consumes a stale entry. **A single `loadProgram(_:)` funnel is not sufficient.** The synchronisation points are: every toolbar undo and redo, every system-driven undo and redo, the close of a coalesced session, and **eviction at the 50-entry bound**, which silently drops a snapshot the manager may still have a registration for.
- [ ] **The bridge mechanism is *executed* before it is specified** (ADR-032 invariant 4), and this story does not prescribe one in advance. The obvious recipe — `removeAllActions(withTarget:)` then register one undo and one redo — **does not work**, and the plan stated it before checking: `UndoManager` puts an ordinary registration on the **undo** side and only treats a registration as *redo* when it is made **while the manager is performing an undo**; an ordinary registration also clears the existing redo stack. So that recipe would register two undos, provide no system redo, and fail this story's own test 7d.
- [ ] Whatever mechanism is chosen must handle **both stacks and system-callback reentrancy** — the manager's redo side can only be populated from inside its own undo execution, which means the bridge has to drive `UndoManager` rather than mirror it, or accept that system redo is unavailable and say so. **Deciding which is part of this story**, and the decision goes in ADR-036 with the evidence that produced it. Note the fallback is not a disaster: the toolbar buttons are the primary surface and work regardless; what is at stake is the system edit-menu and shake affordances.
...
- [ ] **Undo and redo are themselves program mutations, and carry the same consequences an applied edit does** — *without* recording a new undo entry. **Introduced in US-408 with the toolbar pair; re-verified here through the bridge**, so a *system*-driven undo has the same consequences as a toolbar one. `EditAction` has five cases and undo is none of them, so US-405's "an applied action voids the run" hook **does not fire for undo**. Concrete case: edit P→Q, run Q to completion so an export file is prepared, then undo to P — the stale Q run and its prepared `.dst` survive, which is precisely the hazard ADR-038 exists to close. So every successful history transition must **void the run, discard the prepared export, and autosave**, exactly as an applied edit does.
- [ ] Undo does **not** resurrect a previously discarded run or its deleted export file. Undo is an editor affordance; it does not un-cancel a `Task`. Stated explicitly because the opposite is a reasonable thing to assume. It is the *complement* of the criterion above, not a substitute for it.
