...
- [ ] **The bridge mechanism is *executed* before it is specified** (ADR-032 invariant 4), and this story does not prescribe one in advance. The obvious recipe — `removeAllActions(withTarget:)` then register one undo and one redo — **does not work**, and the plan stated it before checking *(Codex round 2)*: `UndoManager` puts an ordinary registration on the **undo** side and only treats a registration as *redo* when it is made **while the manager is performing an undo**; an ordinary registration also clears the existing redo stack. So that recipe would register two undos, provide no system redo, and fail this story's own test 7d.
