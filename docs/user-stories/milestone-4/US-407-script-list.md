...
- [ ] **Every `Formula` case renders as readable text** rather than as a placeholder — `.number`, `.variable`, `.binary` (e.g. `360 ÷ Inner Loop`) **and `.unaryMinus`**, the fourth shipped case. `OctagonRosette` ships two `.binary` nodes, so this is on screen the first time anyone opens that sample. The renderer switches exhaustively over `Formula` with no `default:`, so a fifth case would be a compile error. Rendering is this story's; which of them are *editable* is US-410's.
