...
2. **The `SettlingProgress` shim's captured closure holding a `fitted` from the previous layout pass.** `StageCanvas` documents that staleness and argues it is safe because a manipulation and a fit animation cannot coexist. **A keyboard resize is neither**, so that argument does not cover this case — and this is the suspect the existing code most nearly admits to.

## Acceptance criteria

- [ ] **A device recording of both conditions** — focus the name field from a fresh fit, and after a zoom/pan — with the frame at which the mismatch appears identified. Owner: Sebastian; this cannot be done from here.
- [ ] **A verdict on each of the two remaining suspects**, each either confirmed with evidence or refuted with the reason, appended to this file in the style of the "already ruled out" section above. A suspect left unexamined is recorded as unexamined.
...
2. If the cause turns out to be the `SettlingProgress` stale `fitted`, a test that drives a layout change **while** a fit animation is in flight, which is the combination `StageCanvas`'s doc comment argues cannot happen.

## References

- ADR-028 (the transient class, invisible to stills), ADR-029/ADR-030 (the frame instrumentation available for captures)
- `StageCanvas` — the `SettlingProgress` doc comment whose safety argument does not cover a keyboard resize
