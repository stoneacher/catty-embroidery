...
2. **The `SettlingProgress` shim's captured closure holding a stale value from the previous layout pass.** `StageCanvas.swift:92-104` documents the staleness and is worth reading before starting, **because the backlog entry described its reasoning wrongly and this story inherited that** *(Codex round 1)*. The comment does **not** argue "a manipulation and a fit animation cannot coexist" — it explicitly says that argument **is false** and was already retracted (`swift-code-reviewer`, Q3). Its live argument is narrower: every mutation of `manipulation` goes through the `@State` setter, so the outer body is re-evaluated and the closure rebuilt before the next render, and `baseline(fitting:settlingAt:)` ignores progress once the phase is not `.settling`.

   So the hypothesis has to be restated rather than inherited: **does a keyboard-driven layout change rebuild that closure?** It is not a `manipulation` mutation, so the stated mechanism does not obviously cover it. That is a real question; the first draft's version of it cited a retracted argument and proposed a test against a claim the code does not make.
...
- [ ] **An explicit outcome even if neither recording reproduces it.** "Not reproduced" is a permitted and useful result, and it ends the timebox: record the two conditions tried, the sampling rate, and a recommendation (close as not-reproducible, or keep open with the next condition named). The first draft required identifying a mismatch frame unconditionally, which is unsatisfiable when the transient does not appear *(Codex round 1)*.
- [ ] **A verdict on each of the two remaining suspects** — confirmed with evidence, refuted with a reason, or **explicitly recorded as not examined and why**. Those three are the permitted outcomes; "unexamined" is an outcome, not a gap, provided the timebox is what stopped it. *(The first draft both demanded a verdict and permitted "unexamined", which read as contradictory; Codex round 1.)*
