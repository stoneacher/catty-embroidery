...
- [ ] Applying any action to any program **either** returns a `Program` that passes `Script.validate()` on every script **or** returns `.rejected`. There is no third outcome, and this is the criterion exit criterion 4 is built from.
...
10. **The invariant property**: over a generated sequence of actions applied to a seed program, every `.applied` result passes `Script.validate()`. Seeded deterministically so a failure is reproducible.
