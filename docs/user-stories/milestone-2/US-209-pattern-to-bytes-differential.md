...
## Acceptance criteria
- [ ] The square program's (US-207) `assembledStream()` feeds `DSTFile(stream:name:)`; the emitted bytes are byte-diffed (reusing the M1 `ByteDiff` helper) against a committed golden fixture.
...
## Test-first plan
1. Square program → `DSTFile.data` equals the golden `.dst` fixture (loaded via `Bundle.module`), byte-for-byte via `ByteDiff`.
