...
## Acceptance criteria
- [x] Hardcoded program: `whenStarted` → `setThreadColor` → `runningStitch(length)` → `repeatLoop(4)[moveNSteps(side), turnRight(90)]` → `sewUp` → `writeEmbroideryToFile` — exercising color, pattern activation, loop compilation, motion, tie-off, and the finalize marker in one program.
- [x] Golden ordered `stitch` events **and** golden `assembledStream()`. Expected values are derived from the engine's own pattern types in a test helper — never from the interpreter under test (the M1 oracle discipline: the reference implementation, not the code being verified, produces the expectation).
- [x] Incremental consumption on a real program: driving `step()` one tick at a time yields the identical event sequence and assembled stream as `run()` (the roadmap's batch-equivalence exit test, now on a non-trivial program).
- [x] Determinism: two runs of the same program produce identical events and streams.
