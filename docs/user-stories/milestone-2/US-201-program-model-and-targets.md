...
- [ ] `Program`, `Scene`, `Object`, `Variable` as `Sendable`, `Equatable`, `Codable` value types. `Program` carries a `formatVersion: Int` (ADR-003 versioned format). `Script`/`ScriptHeader` and `Object`'s `scripts` array deliberately land in US-203 together with the `Brick` type they contain, so every story compiles on its own — this story ships the script-free spine.
...
## Test-first plan
1. Codable round-trip: encode → decode a nested `Program` (scene, two objects with variables) and assert whole-value equality (ADR-006 discipline).
...
## References
- `Catroid/.../content/Project.java`, `Scene.java`, `Sprite.java`
