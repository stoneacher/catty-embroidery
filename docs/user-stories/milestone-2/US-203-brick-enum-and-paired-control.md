...
## Acceptance criteria
- [ ] `Script { header: ScriptHeader, bricks: [Brick] }` and `ScriptHeader` (M2: only `.whenStarted` — Catroid's `StartScript`, whose `WhenStartedBrick` header contributes no action) land here, alongside the `Brick` type they contain; `Object` gains its `scripts: [Script]` array (deferred from US-201 so every story compiles on its own).
