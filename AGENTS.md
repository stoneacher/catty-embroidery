...
- The `swift-engineering` Codex plugin is installed — consult its skills when writing Swift (`swift-testing`, `modern-swift`, `swift-style`, `swiftui-patterns`, `ios-hig`, `localization`, `swift-diagnostics`) and its agents for planning/review. The Sosumi MCP server provides live Apple docs.
...
The sibling checkouts (one level up, read-only; access granted via `.Codex/settings.local.json`):
